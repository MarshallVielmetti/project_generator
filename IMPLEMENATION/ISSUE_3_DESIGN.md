...
- `starter.exclude` removes exact paths and their descendants from an
  ...
  teaching contracts, instructor/private/generation material, and
...
- Symlinks and special files are never followed or copied. Existing path
  ...
  output symlink.
- Every directory prefix participates in collision detection, not only leaf
  files. This catches duplicate output files and case-colliding parent
  directories before different filesystems can produce different artifacts.

## Staging and recovery

- Builds run below the owned `build/` root and take an exclusive lock at
  `build/.startergen.lock`. The lock path itself is no-follow validated and
  cannot be selected as a destination. The lock covers enumeration,
  transformation, manifest creation, and promotion.
...
- Cleanup of the old backup is best effort after the new output is committed;
  if cleanup fails, the build still succeeds with the backup retained for
  recovery rather than reporting failure after visible state has changed.
...
- The manifest itself is intentionally excluded from its `files` hash list so
  ...
  Contract bytes are parsed from the same snapshots that are hashed, and each
  allowlisted source is copied while being hashed, so provenance describes the
  bytes that entered staging rather than a later reread.
...
- Each declared exercise source must be present in the allowlist. Exercise
  targets are matched by their declared relative path, with case-only
  normalization where the filesystem permits it, never by inode identity.
  They are grouped by their copied relative source path and passed to the
  existing LibCST transformer inside staging.
- Staged files are temporarily writable while transformation runs, then
  their canonical mode bits are restored before the manifest is generated.
